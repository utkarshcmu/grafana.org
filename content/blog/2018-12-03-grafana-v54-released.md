+++
title = "Grafana v5.4 Released"
author = "mefraimsson"
date = "2018-12-03"
keywords = ["Grafana", "Blog", "v5.4", "Stable", "Release", "Download"]
cover_opacity = "0.4"
cover_blur = "1px"
description = "Grafana 5.4 Stable Release"
...
{{< docs-imagebox img="/img/docs/v54/alerting-for-dark-theme.png" >}}

In the screenshot you can see an example timeline of an alert using the `For` setting. At ~16:04 the alert state changes to `Pending` and after 4 minutes it changes to `Alerting` which is when alert notifications are sent. Once the series falls back to normal the alert rule goes back to `OK`. [Learn more](http://docs.grafana.org/alerting/rules/#for).

Additionally, there's now support for disable the sending of `OK` alert notifications. [Learn more](http://docs.grafana.org/alerting/notifications/#disable-resolve-message).
...
{{< docs-imagebox img="/img/docs/v54/stackdriver_template_query.png" >}}
...
This has the advantage of not needing to generate a private key file for the service account and also not having to upload the file to Grafana. [Learn more](http://docs.grafana.org/features/datasources/stackdriver/#using-gce-default-service-account).

Please read [Using Google Stackdriver in Grafana](http://docs.grafana.org/features/datasources/stackdriver/) for more detailed information on how to get started and use it.

## MySQL Query Builder

Grafana v5.4 comes with a new graphical query builder for MySQL. This brings MySQL integration more in line with some of the other datasources and makes it easier for both advanced users and beginners to work with timeseries in MySQL. Learn more about it in the [documentation](http://docs.grafana.org/features/datasources/mysql/#query-editor).
...
{{< docs-imagebox img="/img/docs/v54/graph_time_regions.png" >}}
...
{{< docs-imagebox img="/img/docs/v54/graph_dot_single_point.png" >}}
